...
# pcaeigvec = np.zeros(28,1540000)

x_train = mnist.train.images[:55000,:]
x_train.shape

image = x_train[1].reshape([28,28])
eigvec,eigval = pca(image)
eigvec = eigvec.reshape(1,28*28)
pcaeigvec = eigvec

for img in range(2,(numberofimages+1)):
    image = x_train[img].reshape([28,28])
    eigvec,eigval = pca(image)
    #pcaeigvec[img] = eigvec
    eigvec = eigvec.reshape(1,28*28)
    pcaeigvec = np.append(pcaeigvec,eigvec,axis=0)
    

# np.set_printoptions(threshold=np.inf, linewidth=np.inf)  # turn off summarization, line-wrapping
# with open(path, 'w') as f:
# f.write(np.array2string(pcaeigvec, separator=', '))

print(pcaeigvec.shape)


x_train = pcaeigvec


randomNum = random.randint(1,(numberofimages+1))
...
sess.run(tf.global_variables_initializer())
iterations = numberofimages/batch_size
for i in range(iterations-1):
    z_batch = np.random.normal(-1, 1, size=[batch_size, z_dimensions])
    print(mnist.train.images.shape)
    real_image_batch = pcaeigvec[i*batch_size:(i+1)*batch_size]
    print(real_image_batch.shape)
    real_image_batch = real_image_batch.reshape(batch_size,28,28,1)
